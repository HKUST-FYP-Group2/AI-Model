...
class Fetcher(BaseFetcher):
    def __init__(self, url: str, limit: int, range: float):
        super().__init__(url)
        self.limit = limit
        self.range = min(range, 250.0)
    ...
    def fetch(self, key=None, lat: float = None, lon: float = None):
        if not lat or not lon:
            logger.error("Please provide both lat and lon")
            return [], False
        _param = {"limit": self.limit,
                  "nearby": f"{lat},{lon},{self.range}", "include": "urls"}
        logger.info(f"Calling the api {self.url}, passed params {_param}")
