...
class CustomLoss(nn.Module):    
    ...
    def forward(self, outputs:torch.Tensor, targets: torch.Tensor, specialCase:str):
        ...
        for index, case_type in enumerate(specialCase):
            current_classification = decimal_to_pentanary(torch.argmax(outputs[index], dim=0).item())
            ...
            if case_type ==  "rain":
                ...
                calm_stormy_val = 3 if dry_wet_val == 2 else (4 if dry_wet_val >= 3 else calm_stormy_val)
            elif case_type == "rime":
                cold_hot_val = 0
            elif case_type == "snow":
                cold_hot_val = 0
            ...
            new_targets.append(cold_hot_val + 5*dry_wet_val + 25*clear_cloudy_val + 125*calm_stormy_val)
